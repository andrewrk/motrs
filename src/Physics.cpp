...
void Physics::squareAndCircle(double cx1, double cy1, double a1, double cx2, double cy2, double r2, double & out_dx, double & out_dy) {
    ...
    // 3 4 5
    // 6 7 8
    int zoneX = 1 * ((cx1 < cx2) + (cx1 + a1 < cx2));
    int zoneY = 3 * ((cy1 < cy2) + (cy1 + a1 < cy2));
    int zone = zoneX + zoneY;
    switch (zone) {
    case 0:
        pointAndCircle(cx1, cy1, cx2, cy2, r2, out_dx, out_dy);
        break;
    case 1:
        out_dx = 0.0;
        out_dy = Utils::min(0.0, cy1 - (cy2 + r2));
        break;
    case 2:
        pointAndCircle(cx1 + a1, cy1, cx2, cy2, r2, out_dx, out_dy);
        break;
    case 3:
        out_dx = Utils::min(0.0, cx1 - (cx2 + r2));
        out_dy = 0.0;
        break;
    case 4:
        // omg inside the square! gtfo!
        pointAndCircle(cx1 + a1 * 0.5, cy1 + a1 * 0.5, cx2, cy2, r2 + a1, out_dx, out_dy);
        break;
    case 5:
        ...
        break;
    case 6:
        pointAndCircle(cx1, cy1 + a1, cx2, cy2, r2, out_dx, out_dy);
        break;
    case 7:
        out_dx = 0.0;
...
void Physics::pointAndCircle(double px1, double py1, double cx2, double cy2, double r2, double &out_dx, double &out_dy) {
    ...
    if (distance < r2) {
        ...
        out_dx = cx2 - normX * r2 - px1;
        out_dy = cy2 - normY * r2 - py1;
    }
}
